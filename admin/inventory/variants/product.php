...
$sql = "SELECT pv.variant_id, p.product_name, c.color, s.size, pv.quantity 
        FROM product_variant pv 
        INNER JOIN product p ON pv.product_id = p.product_id
        INNER JOIN product_color c ON pv.color_id = c.color_id 
        INNER JOIN product_size s ON pv.size_id = s.size_id 
        WHERE pv.product_id = ?";
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    echo "
                                        <tr>
                                            ...
                                            <td>$row[size]</td>
                                            <td>$row[color]</td>
                                            <td><input type='number' value='$row[quantity]' onchange='updateQuantity($row[variant_id], this.value)'></td>
                                            <td><a href='delete.php?variant_id=$row[variant_id]&product_id=$product_id' class='delete-var'><ion-icon name='trash-outline'></ion-icon></a></td>
                                        </tr>
                                    ";
                                }
                                ?>
