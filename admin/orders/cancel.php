        ...
        <div class="action-container top">
            <div class="search">
                <form action="cancel.php" method="GET">
                    <div class="form-input">
                        <input type="search" name="order_id" placeholder="Search">
            ...
            <div class="orders-nav">
                <div class="action">
                    ...
                    <a href="receive.php">Ready <span>(<?php echo $count_ready; ?>)</span></a>
                    <a href="completed.php">Completed <span>(<?php echo $count_completed; ?>)</span></a>
                    <a href="cancel.php" class="active">Cancelled <span>(<?php echo $count_cancelled; ?>)</span></a>
                </div>
            </div>
        </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php
                                ...
                                // Your SQL query
                                $sql = "SELECT 
                                    oc.order_id, oc.order_date, oc.order_cancelled_date,
                                    CONCAT(u.firstname, ' ', u.lastname) AS full_name,
                                    SUM(oc.quantity) AS total_quantity,
                                    SUM(oc.subtotal) AS total_cost,
                                    u.user_id
                                FROM 
                                    order_cancelled oc
                                JOIN 
                                ...
                                if (!empty($_GET['order_id']) && $_GET['order_id'] !== 'all') {
                                    $sql .= " WHERE oc.order_id = ?";
                                }

                                $sql .= " GROUP BY 
                                    oc.order_id, full_name
                                ORDER BY
                                    oc.order_id DESC";
                                ...
                                $orderCancelledDate = strtotime($row['order_cancelled_date']);
                                $orderCancelledDateFormatted = date('F j, Y', $orderCancelledDate);
                                $orderCancelledDateTime = date('g:i a', $orderCancelledDate);
                            ...
                            <tr>
                                ...
                                <td>{$orderCancelledDateFormatted} - {$orderCancelledDateTime}</td>
                            </tr>";
                                }
                                ?>
                            </tbody>
